...
def get_args():
    ...
    parser.add_argument('--actor_rate', type=float, default=1.0)
...
def main():
    ...
    while True:
        ...
        buffer.reset()
        for t in pbar:
            q_value, order_state, p_log = worker.observe(demand.current_demand, t, exploration_rate)
            ...
            worker.update(p_log, feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, assignment, order_state, (t==args.max_step - 1), j)
            assignment = [x for x in assignment if x is not None]
            demand.pickup(assignment)
            demand.update()

        loss = worker.train(args.batch_size, args.train_times, True)
        ...
        if j % args.eval_episode == 0 :
            ...
            pbar = tqdm.tqdm(range(args.max_step))
            for t in pbar:
                q_value, order_state, p_log = worker.observe(demand.current_demand, t)
                assignment, _ = assign(q_value)
                feedback_table, new_route_table, new_route_time_table, new_remaining_time_table, new_total_travel_time_table, new_detour_table, assignment, reward = \
                ...
                worker.update(p_log, feedback_table, new_route_table, new_route_time_table, new_remaining_time_table,
